...
addXrayClient(){
    echo "Generating Xray client configuration..."

    # Load Xray server keys and details
    if [[ -f /root/antizapret/xray_keys ]]; then
        source /root/antizapret/xray_keys
    else
        echo "Error: Xray server keys not found. Please run setup.sh first."
        exit 1
    fi

    # Load client-side routing rules
    if [[ -f result/xray_client_routing_rules.json ]]; then
        CLIENT_ROUTING_RULES=$(cat result/xray_client_routing_rules.json)
    else
        echo "Error: Xray client routing rules not found. Please run doall.sh first."
        exit 1
    fi

    # Construct the full Xray client config JSON
    XRAY_CLIENT_CONFIG="{
  \"log\": {
    \"loglevel\": \"warning\"
  },
  ...
  \"dns\": {
    \"servers\": [
      \"${SERVER_IP}#${KNOT_RESOLVER_PORT}\"
    ]
  },
  \"routing\": {
    \"domainStrategy\": \"IPIfNonMatch\",
    \"rules\": ${CLIENT_ROUTING_RULES}
  }
}"

    echo "\n-----------------------------------------------------------------------------------------------------"
    echo "Copy the JSON configuration below and paste it into your v2box client:"
    echo "-----------------------------------------------------------------------------------------------------"
    echo "${XRAY_CLIENT_CONFIG}"
    echo "-----------------------------------------------------------------------------------------------------"
    echo "\nInstructions for v2box:"
    echo "1. Open v2box and go to 'Profiles'."
    echo "2. Tap the '+' button and select 'Import from JSON'."
    echo "3. Paste the copied JSON configuration."
    echo "4. Save the profile and activate it."
    echo "5. Ensure v2box is running in VPN mode to capture all traffic."
    echo "6. Verify split tunneling: blocked sites should work, non-blocked sites should show your original IP."
    echo "-----------------------------------------------------------------------------------------------------"
}
...
if ! [[ "$OPTION" =~ ^[1-9]$ ]]; then
	...
	echo '    9) Xray - Generate client configuration'
	until [[ "$OPTION" =~ ^[1-9]$ ]]; do
...
case "$OPTION" in
	...
	8)
		...
		;;
	9)
		echo 'Xray - Generate client configuration'
		addXrayClient
		;;
esac
