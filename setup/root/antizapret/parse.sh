...
generate_xray_client_routing_rules() {
    echo "Generating Xray client routing rules..."

    ROUTING_RULES="[]"

    ROUTING_RULES=$(echo "$ROUTING_RULES" | jq '. += [{"type": "field", "ip": ["10.30.0.0/15"], "outboundTag": "proxy"}]')

    ROUTING_RULES=$(echo "$ROUTING_RULES" | jq '. += [{"type": "field", "ip": ["0.0.0.0/32"], "outboundTag": "block"}]')

    if [[ -f result/exclude-hosts.txt ]]; then
        EXCLUDE_DOMAINS=$(cat result/exclude-hosts.txt | jq -R . | jq -s .)
        ROUTING_RULES=$(echo "$ROUTING_RULES" | jq --argjson domains "$EXCLUDE_DOMAINS" '. += [{"type": "field", "domains": $domains, "outboundTag": "direct"}]')
    fi
...
if [[ -z "$1" || "$1" == "host" || "$1" == "hosts" ]]; then
	...
	sed '/^\.$/ s/.*/*. A 10.30.0.1./; t; s/$/ A 10.30.0.1./; p; s/^/*./' result/include-hosts.txt >> result/proxy.rpz
